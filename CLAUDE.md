...
- **Notes** `app/(tabs)/notes.tsx`, the letter wall (redesigned): composer card at TOP (serif input whose placeholder is a rotating "spark" starter; ✦ Sparkles button deals another, Smile toggles the emoji palette, Lock seals a capsule, wax-seal ♥ disc sends). Wall cards carry a tiny deterministic tilt (`tiltFor(id)`, pinned cards sit straight on blush), avatar + relative-time attribution, "✦ Keep on top" pin, and HEART reactions: you can heart only your partner's notes; your own note instead shows "♥ {name} loved this" once they do (`note_hearts` v9, PATCH /api/notes/:id { hearted }, settles on the server response).
...
- **Story chapters** (in `home.ts`): the second retention hook. Everything durable the couple keeps writes "pages" (memories x5, accepted dates x5, prompt answers x3, bucket done x3, notes x2, milestones x2, comments x1); `CHAPTERS` thresholds name the chapter (First Glance -> Ever After, 10 of them). Computed on read from count(*) subqueries in the home aggregate, no counter column, cannot drift. Home renders a quiet card (chapter ordinal + name, 3px accent progress rule, "N pages to Chapter X"). No points UI language anywhere: pages and chapters only.
