...
- **`/api/auth/me`** returns `encryption: boolean`; the client threads it through `useAuth().encryption` to show the `LockBadge` on composers and the Settings → Privacy copy (never overclaim when the key is absent).
...
Wax-seal FAB anchored bottom-right (offset above the tab bar + `env(safe-area-inset-bottom)`), rendered once in `app/(tabs)/_layout.tsx`. Tapping fans a quarter-arc of five labelled actions (Add memory/note/wishlist item, Propose a date, Add milestone) with staggered fade+scale, no bounce; dismiss on tap-outside or swipe-down (scrim `PanResponder`). Visible only on the 5 main tab routes (`usePathname` gate); composers/viewers are RN `Modal`s that portal above it, so it is hidden while they are open. Each action `router.navigate`s to the target with a fresh `compose` nonce; the target screen opens its composer via `useComposeParam` (`src/lib/useComposeParam.ts`) — wishlist always opens on the user's own list. The tab bar itself now reads `useSafeAreaInsets().bottom` for height + paddingBottom (fixes the iOS PWA home-indicator overlap).
...
- **P1.5 theme presets — DEFERRED.** Not started. Reason: real runtime theme switching needs colors resolved at render time, but every screen bakes `colors` from `src/theme.ts` into module-scope `StyleSheet.create`, so it requires an app-wide refactor to a theme context (or dynamic `useThemedStyles`) across ~30 files; persisting `theme_preset` without actually recolouring would be a stub (forbidden). Approach when resumed: add `theme_preset TEXT` on `users`; define 5 full token sets in `src/theme.ts` (keep radius/spacing/type constant); add a `ThemeProvider` exposing the active `colors`; migrate screens to read colors from context (or a `makeStyles(colors)` helper); Settings preview cards render a mock memory tile per preset. Default stays parchment/oxblood/gold. All presets light, WCAG AA.
- **P1.6 comments on memories — DEFERRED.** Not started (blocked behind the same session-budget line as P1.5). Approach when resumed: additive `memory_comments (id, memory_id, author_id, couple_id, body_ct BYTEA, created_at, edited_at)`; body encrypted via `encryptField`/`readField` (already available); couple_id-scoped reads/writes; author-only edit/delete (mirror the hearts rule — never delete the other's); per-memory Ably channel with ciphertext payloads (or server-decrypt before publish); thread UI under the memory detail view, Fraunces body; add a `memory_commented` notification kind through the existing `notify()` path.
- **P2.7 streak system — DEFERRED.** Not started. Approach when resumed: on the couple row add `current_streak_days`, `longest_streak_days`, `last_streak_date DATE`, `grace_used_this_week`, plus a declared timezone; advance the counter on prompt reveal (both answered) using the earlier of the two partners' timezones; one grace day/week; reset to zero (preserve peak) when a day lapses with grace exhausted; Home header chip + a quiet calendar history view; copy: "You paused. Start again whenever." / "Grace day used. Streak continues." No confetti, no leaderboards.
